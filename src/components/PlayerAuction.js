...
function PlayerAuction() {
  ...
  const placeBid = () => {
    if (bidAmount && Number(bidAmount) > currentBid) {
      socket.emit('placeBid', { amount: Number(bidAmount), userId: 'current-user-id' });
      setBidAmount('');
    } else {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {selectedPlayer && auctionStatus !== 'in progress' && (
        <div className="mb-4">
          <h3 className="text-lg font-semibold">Selected Player: {selectedPlayer.name}</h3>
          <button 
      ...
      {auctionStatus === 'in progress' && (
        <div className="mb-4">
          <h3 className="text-lg font-semibold">Auction in Progress</h3>
          <p>Player: {selectedPlayer.name}</p>
          <p>Current Bid: ${currentBid}</p>
          <p>Current Bidder: {currentBidder || 'None'}</p>
